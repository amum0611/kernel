...
/*                    if (!incomingChildPath.startsWith(incomingParentPath)) {
        ...
        // all the non-restored children would be cleared
        String[] allChildPaths =  resourceDAO.getChildren((CollectionImpl) resourceImpl, 0, -1);
        for (String childPath: allChildPaths) {
            if (restoredChildPaths.contains(childPath)) {
                // so it was just restored, so don't clear
                continue;
            }
