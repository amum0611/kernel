...
//            We need to set the tenant ID for current session. Otherwise the underlying operations fails
    ...
    /**
     * Method to set the cache invalidator service used. This method is called when the OSGi cache
     * invalidator service is available.
     *
     * @param invalidator the cache invalidator service.
     */
    protected void setCacheInvalidator(CacheInvalidator invalidator) {
        updateCacheInvalidatorService(invalidator);
    }

    /**
     * This method is called when the current cache invalidator service becomes un-available.
     *
     * @param invalidator the current cache invalidator service instance, to be used for any
     *                    cleaning-up.
     */
    @SuppressWarnings("unused")
    protected void unsetCacheInvalidator(CacheInvalidator invalidator) {
        updateCacheInvalidatorService(null);
    }

    // Method to update cache invalidator service.
    private static void updateCacheInvalidatorService(CacheInvalidator service) {
        invalidator = service;
    }
     ...
     *
     * @return the cache invalidator instance.
     */
