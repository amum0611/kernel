...
public class CheckInCommand {
    ...
    private void createMetaElement(XMLStreamWriter xmlWriter, String filePath, String path,
        ...
        if (file.isDirectory()) {
            ...
            createMetaElementForChild(xmlWriter, filePath, path, callback);
        }
    }
    ...
    private void createDirectoryMetaElement(XMLStreamWriter xmlWriter, String filePath, String path,
        ...
        createMetaElementForChild(xmlWriter, metaDirectoryPath, path, callback);
    ...
    private void createMetaElementForChild(XMLStreamWriter xmlWriter,
        ...
        File metaDirFile = new File(metaFilePath);
        ...
        for(String childMetaFileName : metaFiles){
            String fileName = childMetaFileName.substring(1, childMetaFileName.length() - 4);
            String filePath = metaFilePath.
                    substring(0, metaFilePath.lastIndexOf(File.separator)) + File.separator + fileName;
            File resourceFile = new File(filePath);
            // confirm the existence of the meta file.
            String childMetaFilePath = metaFilePath + File.separator + childMetaFileName;
            OMElement metaElement = Utils.getOMElementFromMetaFile(childMetaFilePath);
            if (metaElement == null) {
                return;
            }
            ...
            if((status = metaElement.getAttribute(new QName(DumpConstants.RESOURCE_STATUS))) != null){
                if(DumpConstants.RESOURCE_ADDED.equals(status.getAttributeValue())){
                    ...
                    FileUtils.deleteQuietly(new File(childMetaFilePath));
            ...
            if(status != null && !DumpConstants.RESOURCE_DELETED.equals(status.getAttributeValue())){
                ...
                Utils.updateMetaFile(childMetaFilePath, metaElement);
                metaElement.addAttribute(DumpConstants.RESOURCE_STATUS, status.getAttributeValue(), null);
            }
