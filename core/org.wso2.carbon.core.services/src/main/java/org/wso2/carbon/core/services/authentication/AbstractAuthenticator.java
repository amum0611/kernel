...
public abstract class AbstractAuthenticator extends AbstractAdmin implements ServerAuthenticator {
    ...
    public void authenticate(MessageContext msgContext) throws AuthenticationFailureException {
        ...
        try {
            ...
            log.error("Unable retrieve Realm service", e);
            throw new AuthenticationFailureException
        ...
        String tenantDomain;
        try {
            tenantDomain = getTenantDomain(userNameInRequest, realmService);
        } catch (org.wso2.carbon.user.core.UserStoreException e) {
            if (log.isDebugEnabled()) {
                log.debug("Unable to retrieve tenant domain for user " + userNameInRequest + " this could most " +
                        "probably due to invalid user name", e);
            }
