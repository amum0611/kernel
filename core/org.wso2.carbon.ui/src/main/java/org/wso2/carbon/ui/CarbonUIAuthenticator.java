...
/**
 * This is the interface of the Authenticator to Carbon UI framework. Implement
 * this interface in a bundle and drop-in to the framework and it will be
 * automatically called. The authenticator has the freedom to decide how it does
 * the authentication.
 * 
 * When an authentication request is received by the framework each
 * authenticator will be checked to see who handles the request, i.e. isHandle()
 * method of each authenticator will be called in the priority order given by
 * the getPriority() method. Highest priority authenticator that returns true
 * for the isHandle() method will be given the responsibility of authentication.
 * 
...
public interface CarbonUIAuthenticator {
    ...
    /**
     * This method will check whether given request can be handled by the
     * authenticator. If authenticator is capable of handling given request this method will
     * return <code>true</code>. Else this will return <code>false</code>.
