 ...
 *
 * When he logs out from either of SSO relying party, a SAML2 LogoutRequest is sent to all the relying
 * party service providers who have established sessions with the Identity Provider at that moment.
 * When a relying party receives a logout request, it should validate the request and extract the
 * IdP session index from the request. Then it should identify the sessionId of the corresponding user
 * which represents the session established at the relying party end. Then it removes that session
 * from the validSessionMap and includes it to the invalidSessionsMap. So when a user tries to do some
 * activity thereafter he should be logged-out from the system.
 *
...
public class CarbonSSOSessionManager {
    ...
    /**
     * This hash map is used to maintain a map of valid sessions.
     * IdpSessionIndex is used as the key while the RPSessionId is used as the value.
     */
    private ConcurrentHashMap<String, String> validSessionMap =
            new ConcurrentHashMap<String, String>();

    /**
     * This hash map is used to maintain the invalid sessions.
     * RPSessionIndex is used as the key while IdpSessionIndex is used as the value.
     */
    private ConcurrentHashMap<String, String> invalidSessionsMap =
            new ConcurrentHashMap<String, String>();
