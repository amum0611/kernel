    ...
    public void initialize(String carbonHome) {
		...
		if (cacheConfiguration.getProperty("configuration.file") != null) {
            ...
            isSync = cacheConfiguration.getProperty("configuration.sync") == null ? true : Boolean.
                     parseBoolean(cacheConfiguration.getProperty("configuration.sync"));
            l1Enabled = cacheConfiguration.getProperty("configuration.l1.enabled") == null ? true :
                     Boolean.parseBoolean(cacheConfiguration.
                            getProperty("configuration.l1.enabled"));
            l1LifeSpan = cacheConfiguration.getProperty("configuration.l1.lifespan") == null ? DEFAULT_EXPIRATION_L1 :
                     Long.parseLong(cacheConfiguration.getProperty("configuration.l1.lifespan"));
            clusterName = cacheConfiguration.getProperty("configuration.clusterName") == null ? DEFAULT_CLUSTER_NAME :
                     cacheConfiguration.getProperty("configuration.clusterName");
            configurationFile = cacheConfiguration.getProperty("configuration.jgroupConfigFile");
            
            if (CacheConfiguration.CACHE_MODE_REPLICATED.equals(cacheModeString)) {
                cacheMode = isSync ? CacheMode.REPL_SYNC : CacheMode.REPL_ASYNC;
            } else if (CacheConfiguration.CACHE_MODE_DISTRIBUTED.equals(cacheModeString)) {
                cacheMode = isSync ? CacheMode.DIST_SYNC : CacheMode.DIST_ASYNC;
            } else if (CacheConfiguration.CACHE_MODE_INVALIDATION.equals(cacheModeString)) {
                cacheMode = isSync ? CacheMode.INVALIDATION_SYNC : CacheMode.INVALIDATION_ASYNC;
            } else {
                log.error("Invalid cache mode provided. The available values are local, replicated, " +
