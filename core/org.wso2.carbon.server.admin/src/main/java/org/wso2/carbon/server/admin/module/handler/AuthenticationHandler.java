...
public class AuthenticationHandler extends AbstractHandler {
    ...
    public InvocationResponse invoke(MessageContext msgContext) throws AxisFault {
        ...
        // do not authenticate if invoked thru local transport.or it is a call
        // to a generic service
        if (Constants.TRANSPORT_LOCAL.equals(msgContext.getIncomingTransportName())
                || callToGeneralService(msgContext) || skipAuthentication(msgContext)) {
...
//            PrivilegedCarbonContext.getCurrentContext().setUsername(username);
        }
