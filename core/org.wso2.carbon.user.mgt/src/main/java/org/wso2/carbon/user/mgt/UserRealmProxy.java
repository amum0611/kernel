...
public class UserRealmProxy {
    ...
    public FlaggedName[] getRolesOfUser(String userName) throws UserAdminException {
        try {
            ...
            RoleDTO[] allRoles = admin.getRoleNamesWithDomain();

            List<FlaggedName> flaggedNames = new ArrayList<FlaggedName>();
            Arrays.sort(userroles);
            for (int i = 0; i < allRoles.length; i++) {
                RoleDTO role = allRoles[i];
                //If this role does not belong to the same domain of the user
                if(domain != null && !domain.equals(role.getDomainName())){
                    //Ignore it and continue with the rest
                    continue;
                }

                FlaggedName fname = new FlaggedName();
                fname.setItemName(role.getRoleName());
                fname.setDomainName(role.getDomainName());
                if (Arrays.binarySearch(userroles, role.getRoleName()) > -1) {
                    fname.setSelected(true);
                }
                //flaggedNames[i] = fname;
