    ...
    public LDAPConnectionContext(RealmConfiguration realmConfig) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Connection Name :: " + connectionName + ","
                    + "Connection Password :: " + connectionPassword + "," + "Connection URL :: "
                    + connectionURL);
        }
        ...
        if (realmConfig.getUserStoreProperty(LDAPConstants.PROPERTY_REFERRAL) != null) {
            environment.put("java.naming.referral", realmConfig.getUserStoreProperty(
                    LDAPConstants.PROPERTY_REFERRAL));
        }
