...
public class JDBCUserStoreManager extends AbstractUserStoreManager{
        ...
        try {
            ...
            while (rs.next()) {
                if (i < maxItemLimit) {
                    ...
                    String domain = realmConfig.getUserStoreProperty(
            ...
            if (secUserManager != null) {
                ...
                allUserNames = UserCoreUtil.combineArrays(users, secUserNames);
            } else {
                allUserNames = users;
            }
	...
	public String[] getRoleNames(boolean noHybridRoles) throws UserStoreException {
        ...
        String[] secondaryRoles = getAllSecondaryRoles();
        if (secondaryRoles != null) {
            //remove repeated roles
            List<String> toBeRemoved = new ArrayList<String>();
            for (String secondaryRole : secondaryRoles) {
                for (String name : names) {
                    if (name.equals(secondaryRole)) {
                        toBeRemoved.add(secondaryRole);
                    }
                }
            }
            names = UserCoreUtil.combineArrays(names, secondaryRoles);
            List<String> allRoles = new ArrayList(Arrays.asList(names));
            for (String roleToRemove : toBeRemoved) {
                allRoles.remove(roleToRemove);
            }
            if (allRoles != null) {
                names = allRoles.toArray(new String[allRoles.size()]);
            }
        }
		return names;
	}
    ...
    public String[] getRoleListOfUser(String userName) throws UserStoreException {
        ...
        try {
            //this.addDomainToUserName(userName);
        ...
        if(useOnlyInternalRoles){
        	names = getInternalRoleListOfUser(userName);
        } else {
...
//			if (!domainMatched && !domainProvided) {
//				secondaryRoles = getSecondaryRoleListOfUser(userName);
//			}

			List<String> roles = getExternalRoleListOfUser(userName);
			if (isReadOnly()) {
				Collections.addAll(roles, getInternalRoleListOfUser(userName));
			}
...
//			}
			...
			names = roles.toArray(new String[roles.size()]);
        }
        //add user roles into userRolesCache
        addToUserRolesCache(this.tenantId, addDomainToUserName(userName), names);
        return names;
    }
    ...
    public RoleDTO[] getRoleNamesWithDomain(boolean noHybridRoles) throws UserStoreException{
        ...
        if (useOnlyInternalRoles) {
            String[] hybrids = hybridRoleManager.getHybridRoles();
            names = UserCoreUtil.combineArrays(names, hybrids);
            return UserCoreUtil.convertRoleNamesToRoleDTO(names, domain);
        } else {
            String sqlStmt = realmConfig.getUserStoreProperty(JDBCRealmConstants.GET_ROLE_LIST);
            if (sqlStmt == null) {
                throw new UserStoreException("The sql statement for retrieving role name is null");
    ...
    public void doUpdateRoleListOfUser(String userName, String[] deletedRoles, String[] newRoles)
        ...
        if (useOnlyInternalRoles) {
            userName = this.addDomainToUserName(userName);
