		...
		if (!noHybridRoles) {
            ...
            for (String internalRole : internalRoles) {
                for (String externalRole : externalRoles) {
                    //if roles are repeated in two lists, add them to toBeRemovedList
                    if (externalRole.equals(internalRole)){
                        toBeRemoved.add(externalRole);
                    }
                }
            }
            //combine internal and external roles.
            String[] allRoles = UserCoreUtil.combine(internalRoles, externalRoles);
            List<String> allRolesList = new ArrayList(Arrays.asList(allRoles));
            //remove any repeated roles.
            for (String toBeRemovedRole : toBeRemoved) {
                allRolesList.remove(toBeRemovedRole);
            }
            return allRolesList.toArray(new String[allRolesList.size()]);
		}
...
//                    domainMatched = true;
        ...
        String userNameWithDomain = addDomainToUserName(userName);

        String[] internalRoles = getInternalRoleListOfUser(userNameWithDomain);
...
//        }
        ...
        addToUserRolesCache(this.tenantID, userNameWithDomain, roleList);
