...
public class ReadOnlyLDAPUserStoreManager extends AbstractUserStoreManager {
	 ...
	 * to an external ldap where empty roles not allowed, then following property should be set
	 * accordingly in user-mgt.xml
	 */
	...
	protected void checkRequiredUserStoreConfigurations() throws UserStoreException {
		...
		// Groups properties
		readLDAPUserGroups =
		                     Boolean.parseBoolean(realmConfig.getUserStoreProperty(LDAPConstants.READ_LDAP_GROUPS));
	...
	public boolean doAuthenticate(String userName, Object credential) throws UserStoreException {
		...
		if (patterns != null && !patterns.isEmpty()) {
			...
			if (userDNPatternList.length > 0) {
				...
				if (!bValue) {
					throw new UserStoreException("User: " + userName +
