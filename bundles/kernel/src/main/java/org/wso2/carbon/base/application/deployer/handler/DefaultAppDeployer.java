...
public class DefaultAppDeployer implements AppDeploymentHandler {
    ...
    private void uninstallBundle(String bundlePath){
        ...
        try {
            ...
            log.error("Error while uninstalling bundle : " + bundlePathFormatted);
        }
    }
    ...
    private void deployRecursively(List<Artifact.Dependency> deps, AxisConfiguration axisConfig)
        ...
        for (Artifact.Dependency dependency : deps) {
            ...
            if (SERVICE_META_TYPE.equals(artifact.getType())) {
                destPath = repo + File.separator + SERVICE_META_DIR;
            } else if (AAR_TYPE.equals(artifact.getType())) {
    ...
    private void undeployRecursively(List<Artifact.Dependency> deps,
        ...
        for (Artifact.Dependency dependency : deps) {
            ...
            /**
             * if the current artifact is a lib or bundle, we have to manually uninstall it into the
             * OSGi environment for the usage of the lib before the first restart..
             * Important : This OSGi library uninstallation is only allowed for the super tenant
             */
            if ((artifact.getType().startsWith("lib/") || BUNDLE_TYPE.equals(artifact.getType()))
                    && AppDeployerUtils.getTenantId(axisConfig) ==
                    MultitenantConstants.SUPER_TENANT_ID) {
